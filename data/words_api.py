...
blueprint = Blueprint("words_api", __name__, template_folder="templates")
...
def get_user_words(user_id):
    ...
    user_words_id = (
        db_sess.query(User.words).filter(User.id == user_id).first()[0].split(",")
    )
    words = db_sess.query(Word).filter(Word.id.in_(list(map(int, user_words_id)))).all()
    levels = (
        db_sess.query(Word_level)
        .filter(Word_level.user_id == user_id, Word_level.word_id.in_(user_words_id))
        .all()
    )
    data = {}
    for i in words:
...
def get_words():
    ...
    return jsonify({item.id: item.to_dict(only=("word", "word_ru")) for item in words})
...
def create_word():
    ...
    elif not all(key in request.json for key in ["word", "word_ru"]):
        return jsonify({"error": "Bad request"})
    db_sess = db_session.create_session()
    word = Word(word=request.json["word"], word_ru=request.json["word_ru"])
    db_sess.add(word)
    db_sess.commit()
