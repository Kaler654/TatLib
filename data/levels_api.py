...
blueprint = Blueprint("levels_api", __name__, template_folder="templates")
...
def get_levels():
    ...
    return jsonify({item.id: item.to_dict() for item in levels})
...
def get_one_level(level_id):
    ...
    return jsonify({level.id: level.to_dict()})
