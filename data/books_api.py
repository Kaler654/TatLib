...
blueprint = Blueprint("books_api", __name__, template_folder="templates")
...
def get_books():
    ...
    return jsonify({item.id: item.to_dict() for item in books})
...
def get_one_book(book_id):
    ...
    return jsonify({book.id: book.to_dict()})
...
def create_book():
    if not request.json:
        ...
        key in request.json for key in ["pages", "title", "level_id", "user_author_id"]
    ):
...
def edit_job(book_id):
    if not request.json:
        ...
        key in request.json
        for key in ["author", "user_author_id", "pages", "title", "level_id"]
    ):
