...
blueprint = Blueprint("word_levels_api", __name__, template_folder="templates")
...
def get_word_levels():
    ...
    return jsonify({item.id: item.to_dict() for item in word_levels})
...
def get_one_word_level(word_level_id):
    ...
    return jsonify({word_levels.id: word_levels.to_dict()})
...
def create_word_levels():
    ...
    elif not all(key in request.json for key in ["word_id", "user_id", "word_level"]):
...
def edit_word_level(user_id, word_id):
    if not request.json:
        ...
        key in request.json for key in ["word_id", "user_id", "word_level", "date"]
    ):
    ...
    word_level = (
        ...
        .filter(User.id == user_id, Word_level.word_id == word_id)
        .first()
    )
    if word_level:
